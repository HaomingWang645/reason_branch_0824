#!/usr/bin/env bash
# ============================================================
#  Think3D-RL reproduction: GRPO on Qwen3-VL-4B-Instruct (single GPU 5) — vLLM colocate rollouts
#  (identical recipe to train_grpo_4b.sh; only the rollout engine differs: HF generate -> vLLM)
#
...
# GPU 5 is Exclusive_Process: keep the vLLM engine core in-process (no EngineCore subprocess)
export VLLM_ENABLE_V1_MULTIPROCESSING=0
export VLLM_USE_V1=1
MODEL=$(ls -d /home/haoming/.cache/huggingface/hub/models--Qwen--Qwen3-VL-4B-Instruct/snapshots/*)
OUT="$SPAGENT_ROOT/output/grpo_think3d_4b_vllm"
...
swift rlhf \
    ...
    --vllm_limit_mm_per_prompt '{"image": 12}' \
    --sleep_level 1 \
    --offload_model true \
    --max_grad_norm 0.5 \
