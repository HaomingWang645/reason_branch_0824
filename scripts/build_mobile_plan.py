"""Build ViewTree_Mobile_Jetson_Plan.pdf + report/DESIGN_MOBILE_JETSON.html from DESIGN_MOBILE_JETSON.md
(same styling family as build_design_impl.py, different running header)."""
import os, re, markdown, weasyprint
R = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
md = open(os.path.join(R, "DESIGN_MOBILE_JETSON.md")).read()
body = markdown.markdown(md, extensions=["tables"])
css = """
@page { size: A4; margin: 20mm 18mm 18mm 18mm;
  @top-left { content: "VIEWTREE  |  MOBILE RUNTIME — JETSON PLAN"; font-family: 'DejaVu Sans'; font-size: 7pt; letter-spacing: .12em; color: #666; }
  @bottom-right { content: "Implementation plan  •  " counter(page); font-family: 'DejaVu Sans'; font-size: 7.5pt; color: #666; } }
...
"""
weasyprint.HTML(string=body, base_url=R).write_pdf(os.path.join(R, "ViewTree_Mobile_Jetson_Plan.pdf"), stylesheets=[weasyprint.CSS(string=css)])
html = f"""<title>ViewTree Jetson Runtime Plan</title>
...
open(os.path.join(R, "report/DESIGN_MOBILE_JETSON.html"), "w").write(html)
