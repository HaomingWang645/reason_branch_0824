...
J=0; g(){ echo ${G[$((J % NP))]}; }
CUDA_VISIBLE_DEVICES=$(g) $P $R/scripts/eval_policy_v2.py --adapter $CK $EVAL --split MindCube_tinybench --out $OUT/${NAME}_tiny.jsonl > $LOG/eval_${NAME}_tiny.log 2>&1 & J=$((J+1))
CUDA_VISIBLE_DEVICES=$(g) $P $R/scripts/eval_policy_v2.py --adapter $CK $EVAL --split MindCube_rest_clean --out $OUT/${NAME}_rest.jsonl > $LOG/eval_${NAME}_rest.log 2>&1 & J=$((J+1))
for i in 0 1; do CUDA_VISIBLE_DEVICES=$(g) $P $R/scripts/run_eval.py --condition memory --adapter $CK --parity odd --shard $i --num-shards 2 --out $OUT/${NAME}_vsimem_s$i.jsonl > $LOG/eval_${NAME}_vsimem_s$i.log 2>&1 & J=$((J+1)); done
wait
for i in 0 1 2 3; do CUDA_VISIBLE_DEVICES=$(g) $P $R/scripts/run_tree.py --parity odd --adapter $CK --conf-head $R/checkpoints/conf_head_v2.pt --shard $i --num-shards 4 --out $OUT/${NAME}_tree_s$i.jsonl > $LOG/eval_${NAME}_tree_s$i.log 2>&1 & J=$((J+1)); done
wait
