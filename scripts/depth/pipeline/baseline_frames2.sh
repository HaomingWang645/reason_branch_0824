...
DSE_GPUS=6,7 $TR --nproc_per_node=2 --master_port=29634 scripts/train_sft.py --data data/train3r/frames_only.jsonl --init checkpoints/depth/sft_frames --skip-frac 0.6 --out checkpoints/depth/sft_frames --epochs 1 --lr 8e-5 --accum 16 > $L/train_sft_frames_resume.log 2>&1
