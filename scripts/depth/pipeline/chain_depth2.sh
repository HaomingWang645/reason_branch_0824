...
st "phase1 train SFT-A (resumed on GPUs 4-7)"
DSE_GPUS=$G $TR --nproc_per_node=$NP --master_port=29631 scripts/train_sft.py --data data/train3r/phase1.jsonl --init checkpoints/depth/sft_a --skip-frac 0.06 --out checkpoints/depth/sft_a --epochs 1 --lr 9e-5 --accum 16 > $L/train_sft_a_resume.log 2>&1
grep -aq "^SAVED" $L/train_sft_a_resume.log || { st "SFT-A FAILED"; exit 1; }
st "phase2 oracle walks"
i=0; for g in 4 5 6 7; do CUDA_VISIBLE_DEVICES=$g $P scripts/depth/oracle_walks.py --adapter checkpoints/depth/sft_a --per-scene 30 --beam 2 --depth 3 --shard $i --num-shards 4 --out data/train3r/oracle_s$i.jsonl --feats data/train3r/feats_oracle > $L/oracle_s$i.log 2>&1 & i=$((i+1)); done; wait
...
DSE_GPUS=$G PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True $TR --nproc_per_node=$NP --master_port=29633 scripts/depth/train_grpo_walk.py --init checkpoints/depth/sft_c --out checkpoints/depth/grpo_walk --items 120000 --per-scene 80 --group 6 --accum-items 8 > $L/train_grpo_walk.log 2>&1
