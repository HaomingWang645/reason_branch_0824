...
set -u
cd "$(dirname "$0")/.."
GPUS=${GPUS:-"6 7"}; CHUNKS_PER_GPU=${CHUNKS_PER_GPU:-3}
for cfg in $CONFIGS; do
  mode=${cfg%%:*}; bb=${cfg#*:}
  echo "[matrix] $(date '+%F %T') start $mode $bb"
  MODE=$mode BACKBONE=$bb GPUS="$GPUS" CHUNKS_PER_GPU=$CHUNKS_PER_GPU bash scripts/run_avic.sh
  # re-run any chunk that crashed (OOM etc.); no-op when everything completed
  read -r -a _g <<< "$GPUS"; nchunks=$(( ${#_g[@]} * CHUNKS_PER_GPU ))
