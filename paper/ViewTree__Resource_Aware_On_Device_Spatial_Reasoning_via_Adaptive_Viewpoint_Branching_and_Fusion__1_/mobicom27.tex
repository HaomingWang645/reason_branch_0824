...
\section{Introduction}
	...
	\begin{figure*}[t]
	\centering
	\includegraphics[width=0.94\textwidth]{figures/fig_wm_failure.pdf}
	\vspace{-0.12in}
	\caption{World-model view prediction versus \system's reconstruct-then-render, on a real captured scene. From the current view, the VLM requests ``turn left $\approx$85$^\circ$'' and then ``move forward $\approx$0.7\,m''; both requested views were actually captured in the walkthrough, so the correct outcome of each movement is known. The world model (Stable Virtual Camera~\cite{svc2025}, the world model used by MindJourney~\cite{mindjourney2025}) predicts a bed and walls that contradict the recorded frames, and drifts further when its own prediction is fed back as the input for the second movement. \system instead renders both requested views by re-projecting one reconstruction built from all frames: each view stays consistent with the actual scene, and errors cannot accumulate across movements.}
...
\begin{figure}[t]
	...
	\vspace{-0.28in}
	\caption{Scaling the frame count on Jetson AGX Orin (7B backbone, measured): latency grows superlinearly and memory is exhausted at 512 frames, while the position-embedding limit binds near 128 frames.}
...
\begin{figure}[t]
	...
	\vspace{-0.28in}
	\caption{Two viewpoint proposals on the same reconstruction. Unconstrained proposals view the scene from outside, through the ceiling; \system restricts poses to positions a person could occupy and masks poses without geometric support.}
...
\begin{figure}[t]
	...
	\vspace{-0.28in}
	\caption{A single reasoning path fails on a real question: the most confident first movement (path A) answers 20\,cm vs.\ ground truth 12, and confidence cannot separate it from the runner-up (0.700 vs.\ 0.697). Retaining two paths lets the next observations correct the error.}
...
Table~\ref{tab:vt_main_7B} reports the full benchmark grid with the 7B backbone, and Table~\ref{tab:vt_scale} the backbone-scale study on VSI-Bench; key deltas are given with scene-bootstrap 95\% confidence intervals.
...
\begin{table*}[t]
	...
	\caption{Performance with Qwen2.5-VL-7B. Task acronyms are defined in Section~\ref{sec:experiment_setting}; VSI Avg is the mean over all ten VSI task types, STI Avg over all eight STI tasks. R.Dr.\ averages the easy/medium/hard splits. Static scene memory uses the walk-trained answerer with all five rendered views attached to every question. ``--'': prompting- and render-exposure baselines are compared on VSI-Bench.}
	\vspace{-0.2in}
...
\begin{table}[t]
	...
	\caption{Backbone-scale study on the VSI-Bench held-out split. The trained \system~controller and confidence head are 7B-specific, so 3B/32B use the training-free variant (token-confidence pruning, human-constrained poses). Benchmark-QA SFT/GRPO train on MindCube-style QA~\cite{mindcube2025}; the 30k-subset corpus SFT is the data-matched answerer at 3B (the full corpus SFT does not fit 32B fine-tuning in our budget); static scene memory is zero-shot render exposure. GRPO at 32B was not run.}
	\vspace{-0.2in}
	\label{tab:vt_scale}
\end{table}
% ------------------------------------------------------------------
...
\begin{figure*}[t]
	...
	\vspace{-0.15in}
	\caption{Measured compute efficiency on Jetson AGX Orin (7B backbone; energy integrates the three INA3221 power rails).}
	\vspace{-0.1in}
...
\begin{figure*}[t]
	...
	\vspace{-0.15in}
	\caption{Spatial task performance under different levels of scene complexity (VSI-Bench held-out split, 7B backbone): counting questions binned by ground-truth object count; all questions binned by the scene's room area and by video duration (tercile splits).}
	\vspace{-0.1in}
...
\begin{table}[t]
	...
	\end{minipage}
	\vspace{0.05in}
	\caption{Left: view-acquisition strategies with the same trained answerer (VSI-Bench held-out, average over ten types and VLM calls per question). Right: the training-free tree transplanted across backbone sizes.}
...
\begin{table}[t]
	...
	\end{tabular}
	\caption{Component ablations on the VSI-Bench held-out split (7B): average over the ten task types and VLM calls per question.}
