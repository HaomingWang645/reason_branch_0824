...
\end{figure}
	...
	\begin{figure*}[t]
		...
		\resizebox{0.96\textwidth}{!}{%
			\begin{tikzpicture}[
				>=Latex,
				flow/.style={draw=none, rounded corners=3pt, minimum height=1.08cm,
					align=center, font=\sffamily\small\bfseries, text=white},
				link/.style={draw=blue!60!black, line width=1.05pt, -{Latex[length=2.5mm]}},
				loop/.style={draw=blue!60!black, line width=1.05pt, -{Latex[length=2.5mm]}, rounded corners=2pt}
				]
				\node[flow, fill=blue!65, text width=2.35cm] (history) at (0,2.2) {History frames};
				\node[flow, fill=purple!70, text width=2.65cm] (vggt) at (3.35,2.2) {Frozen VGGT\\reconstruction};
				\node[flow, fill=purple!70, text width=2.65cm] (memory) at (6.85,2.2) {Explicit 3D\\world memory};
				\node[flow, fill=orange!82!black, text width=3.15cm, minimum height=1.35cm] (policy) at (10.65,2.2) {Confidence-guided\\expand / keep\\stop / arbitrate};
				
				\node[flow, fill=blue!65, text width=2.35cm] (query) at (0,0) {Query + current\\camera view};
				\node[flow, fill=blue!45!black, text width=2.8cm, minimum height=1.25cm] (vlm) at (3.35,0) {Fine-tuned VLM\\controller};
				\node[flow, fill=teal!65!black, text width=2.35cm] (camera) at (6.85,0) {Virtual camera\\action};
				\node[flow, fill=teal!65!black, text width=3.0cm] (render) at (10.65,0) {Render + encode\\new viewpoint};
				
				\draw[link] (history) -- (vggt);
				\draw[link] (vggt) -- (memory);
				\draw[link] (query) -- (vlm);
				\draw[link] (vlm) -- (camera);
				\draw[link] (memory) -- (camera);
				\draw[link] (camera) -- (render);
				\draw[link] (render) -- (policy);
				\draw[link] (policy) -- (memory);
				\draw[loop] (render.south) |- ($(vlm.south)+(0,-0.85)$) -- (vlm.south);
				\node[font=\sffamily\small\bfseries, text=black!65] at (5.35,-1.18)
				{Closed-loop active spatial reasoning over an explicit scene memory};
		\end{tikzpicture}}
...
\end{figure}
	...
	Each successor also needs a score that says how promising it is. Verbalized self-confidence and answer-token probability are known to be poorly calibrated for this purpose; instead, a small two-layer head reads the VLM's internal state at the answer and predicts the probability that this answer is correct. The head is trained on the outcomes of training-time search (Section~\ref{sec:training}) and calibrated on held-out scenes, so its scores are comparable across paths and depths. After expanding all active paths, \system simply keeps the two successors with the highest calibrated confidence. Retained paths share the question and context frames, but their acquired views and later reasoning remain separate, so one early wrong turn cannot contaminate a sibling path.
...
\end{figure} We first teach the VLM to answer from the original frames and newly rendered views. We then use oracle search to produce camera-action and stopping targets. Finally, we use the states visited during oracle search to train the confidence head. The optional reinforcement-learning stage improves answer prediction but not camera control in our experiments; therefore, the final system uses the controller trained from oracle trajectories.
	...
	Concretely, an answer counts as correct when its evaluation score (exact match for multiple choice, relative accuracy for numerical answers) exceeds one half. If the VLM already answers correctly from the original frames, the selected trajectory is empty and the target action is \textsc{Stop}; these depth-zero examples are the most frequent labels and are what teach the controller not to over-explore. Otherwise, among all tested trajectories whose final answer is correct, the search keeps the one with the fewest camera movements, breaking ties by how strongly the VLM favors the correct answer at the end (Figure~\ref{fig:training_pipeline}, middle). Preferring the shortest successful trajectory bakes the acquisition cost into the supervision itself: the controller is never shown a longer walk when a shorter one suffices.
	...
	The selected oracle trajectory teaches the controller both where to move and when to stop: at every prefix of the trajectory the controller imitates the next oracle action, and at the final state it imitates \textsc{Stop}. Invalid movements are removed before action prediction, so the controller only ever chooses among camera actions that can actually be executed from the current pose.
